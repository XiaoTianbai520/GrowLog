...
with sync_playwright() as playwright:
    ...
    # A context menu acts on its target without replacing the open note.
    first_item = page.locator('.note-list-item').filter(has_text='让记录成为日常')
    first_item.click(button='right')
    menu = page.get_by_role('menu', name='管理笔记 让记录成为日常')
    expect(menu).to_be_visible()
    expect(page.get_by_label('笔记标题', exact=True)).to_have_value('本周阅读清单')
    page.keyboard.press('Escape')
    expect(menu).to_have_count(0)
    first_item.click(button='right')
    page.get_by_role('menuitem', name='取消收藏', exact=True).click()
    expect(first_item.locator('svg')).to_have_count(0)
    assert next(n for n in invoke('bootstrap')['notes'] if n['id'] == first_id)['favorite'] is False
    first_item.click(button='right')
    page.get_by_role('menuitem', name='重命名', exact=True).click()
    rename_dialog = page.get_by_role('dialog', name='重命名笔记')
    rename_dialog.get_by_label('笔记标题', exact=True).fill('让记录成为日常（已重命名）')
    rename_dialog.get_by_role('button', name='保存', exact=True).click()
    expect(rename_dialog).to_have_count(0)
    renamed_item = page.locator('.note-list-item').filter(has_text='让记录成为日常（已重命名）')
    expect(renamed_item).to_be_visible()
    renamed_item.click(button='right')
    page.get_by_role('menuitem', name='移到回收站', exact=True).click()
    expect(renamed_item).to_have_count(0)
    page.locator('.trash-filter').click()
    trashed_item = page.locator('.note-list-item').filter(has_text='让记录成为日常（已重命名）')
    trashed_item.click(button='right')
    expect(page.get_by_role('menuitem', name='永久删除', exact=True)).to_be_visible()
    page.get_by_role('menuitem', name='恢复', exact=True).click()
    expect(trashed_item).to_have_count(0)
    page.locator('.note-filters').get_by_role('button', name='全部笔记', exact=False).click()
    expect(page.locator('.note-list-item').filter(has_text='让记录成为日常（已重命名）')).to_be_visible()
    expect(page.get_by_label('笔记标题', exact=True)).to_have_value('本周阅读清单')
